...
- `getAccounts: () => InjectedPolkadotAccount[]`. Get accounts available in that moment in time. [InjectedPolkadotAccount](https://github.com/polkadot-api/polkadot-api/blob/main/packages/pjs-signer/README.md#injectedpolkadotaccount)
- `subscribe: (cb: (accounts: InjectedPolkadotAccount[]) => void) => () => void`. Subscribe function accepting a callback that will be called every time the wallet announces account changes. Returns a function to unsubscribe. [InjectedPolkadotAccount](https://github.com/polkadot-api/polkadot-api/blob/main/packages/pjs-signer/README.md#injectedpolkadotaccount)
...
- `polkadotSigner: PolkadotSigner` ready to use.
