...
Nevertheless, some WS Providers over a Paritytech Polkadot-SDK based node offer a flawed version of the JSON-RPC API Spec, which can be solved in the client in some cases.

If you are using `@polkadot-api/legacy-provider`, DO NOT use `withPolkadotSdkCompat`. `withLegacy` already tackles every bit to offer a spec compliant JSON-RPC server.
