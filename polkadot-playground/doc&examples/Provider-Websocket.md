...
In order to create the provider, you can pass one (or more) websocket uris with its optional supported protocols. For example:
...
Use `innerEnhancer` to pass in enhancers that must be applied before any other enhancers (like legacy-provider).
...
You can even bring your own WebSocket implementation using `websocketClass` if you prefer. By default, it uses the global WebSocket available in browsers, Node.js >=22.4.0, and Bun. For example, for NodeJS 20 and ws library:
