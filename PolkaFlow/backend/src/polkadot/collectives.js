...
import { westend2_collectives } from "polkadot-api/descriptors";
import { CHAIN_CONFIG } from "../config/chains.js";

class CollectivesService {
  constructor(chainId = "westend2_collectives") {
  ...
  async connect() {
    ...
    try {
      ...
      this.api = this.client.getTypedApi(westend2_collectives);
      return this.api;
    } catch (error) {
  ...
  async getXcmFee(destinationChain, amount) {
    ...
    return {
      success: false,
      message: "XCM fee estimation not yet implemented for Collectives",
      chainId: this.config.id,
      destinationChain
    };
  }
...
export { CollectivesService };
export default CollectivesService;