...
class CoretimeService {
  constructor(chainId = "westend2_coretime") {
  ...
  async connect() {
    ...
    try {
      ...
      this.api = this.client.getTypedApi(westend2_coretime);
      return this.api;
    } catch (error) {
  ...
  async getXcmFee(destinationChain, amount) {
    ...
    return {
      success: false,
      message: "XCM fee estimation not yet implemented for Coretime",
      chainId: this.config.id,
      destinationChain
    };
  }
...
export { CoretimeService };
export default CoretimeService;